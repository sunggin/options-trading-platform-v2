...
import { loadTradesHybrid, invalidateTradesCache } from '@/lib/optimizedSupabase'
...
export default function OptimizedTradesTable({ refreshTrigger }: TradesTableProps) {
  const { user } = useAuth()
  ...
  const loadTrades = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('Trades loaded successfully:', data?.length || 0, 'trades')
        setTrades(data || [])
      }
    } catch (error) {
      console.error('Unexpected error loading trades:', error)
    ...
    try {
      ...
      if (['cost', 'strike_price', 'contracts', 'option_type'].includes(field) && currentTrade) {
        ...
        if (optionType === 'Call option' && contracts > 0) {
          ...
          console.log('Auto-calculated PMCC:', pmccCalc, 'for Call option')
        } else {
  ...
  return (
    <div className="max-w-full mb-1">
      ...
      <div className="mt-4 p-2 bg-green-50 border border-green-200 rounded-lg">
        <div className="flex items-center justify-between text-xs">
          <span className="text-green-700">
            ⚡ Trades loaded with optimized caching
          </span>
      ...
      {trades.length > 0 && (
        <div className="card mb-2 w-full mt-4">
